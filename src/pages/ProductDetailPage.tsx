...
import { Star, Heart, Share2, Bell, ExternalLink, Truck, Shield, Clock, ChevronDown, ChevronUp, Check, TrendingDown, Eye, Zap, BarChart3, Copy, CheckCircle } from 'lucide-react';
...
const ProductDetailPage: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/" className="px-6 py-3 bg-indigo-600 text-white rounded-full font-medium hover:bg-indigo-700 transition-colors">Go Home</Link>
        </div>
      </div>
    );
  }
  ...
  const chartData = useMemo(() => {
    ...
    product.priceHistory
      ...
      .forEach(entry => {
        ...
        dateEntry[entry.storeId] = entry.currency === '$' ? entry.price * 1500 : entry.price;
      });
  ...
  const handleAffiliateClick = (listing: typeof product.listings[0]) => {
    ...
    // Simulate redirect - in production this would be: window.location.href = listing.affiliateUrl;
    setTimeout(() => {
      setRedirecting(null);
      addToast(`✅ Click logged! In production, you'd be redirected to ${listing.store.website}`, 'success', 5000);
    }, 1500);
  };

  const handleShare = async () => {
    const url = window.location.href;
    const text = `Check out ${product.name} - Best price: ${formatPrice(bestListing.totalCost, bestListing.currency)} at ${bestListing.store.name}`;
    ...
    if (navigator.share) {
      ...
      try {
        ...
        addToast('Could not copy link. Please try manually.', 'error');
      }
    }
  };
  ...
  const handleWatchlistToggle = () => {
    if (!isAuthenticated) {
      addToast('Please sign in to save items to your watchlist', 'warning');
      navigate('/login');
      return;
    }
    if (inWatchlist) {
      ...
      addToast('Added to watchlist! We\'ll notify you of price changes.', 'success');
    }
  };

  const handleSetAlert = () => {
    if (!targetPrice || parseInt(targetPrice) <= 0) {
      addToast('Please enter a valid target price', 'warning');
      return;
    }
    addToast(`Price alert set! We'll notify you when the price drops below ₦${parseInt(targetPrice).toLocaleString()}`, 'success');
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6 flex-wrap">
          ...
          <span>/</span>
          <Link to={`/search?category=${product.category}`} className="hover:text-indigo-600 transition-colors capitalize">{product.category}</Link>
          <span>/</span>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="aspect-square relative">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                {bestListing.discount > 10 && (
                  <span className="bg-red-500 text-white text-sm font-bold px-3 py-1.5 rounded-full shadow-lg">-{bestListing.discount}% OFF</span>
                )}
                {product.tags.includes('bestseller') && (
                  <span className="bg-yellow-500 text-white text-sm font-bold px-3 py-1.5 rounded-full shadow-lg">BESTSELLER</span>
                )}
              </div>
              <div className="absolute top-4 right-4 flex flex-col gap-2">
                ...
                >
                  {copied ? <Check size={18} className="text-green-500" /> : <Share2 size={18} className="text-gray-600" />}
                </button>
              </div>
                ...
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${
                      selectedImage === i ? 'border-indigo-500 ring-2 ring-indigo-200' : 'border-gray-200 hover:border-gray-300'
                    }`}
                  >
          ...
          <div>
            <div className="mb-2 flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500"><Eye size={12} className="inline" /> {product.totalClicks.toLocaleString()} views</span>
            </div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-5 mb-6">
              ...
              <div className="flex items-end gap-3">
                <span className="text-3xl font-bold text-gray-900">{formatPrice(bestListing.totalCost, bestListing.currency)}</span>
                {bestListing.originalPrice > bestListing.price && (
                  <span className="text-lg text-gray-400 line-through mb-1">{formatPrice(bestListing.originalPrice, bestListing.currency)}</span>
                )}
              </div>
              ...
              <button
                ...
                className="mt-4 w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white font-semibold py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 disabled:opacity-70"
              >
                {redirecting === bestListing.store.id ? (
            ...
            <div className="grid grid-cols-4 gap-3 mb-6">
              <button
                onClick={() => {
                  if (!isAuthenticated) { addToast('Please sign in to set price alerts', 'warning'); navigate('/login'); return; }
                  setShowAlertModal(true);
                }}
                className="flex flex-col items-center gap-1 p-3 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 hover:bg-indigo-50 transition-all"
              >
              ...
              <button
                ...
                className="flex flex-col items-center gap-1 p-3 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 hover:bg-indigo-50 transition-all"
              >
                {copied ? <CheckCircle size={20} className="text-green-600" /> : <Share2 size={20} className="text-indigo-600" />}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2 text-sm text-gray-600"><Truck size={16} className="text-indigo-500" /><span>Free delivery available</span></div>
              <div className="flex items-center gap-2 text-sm text-gray-600"><Shield size={16} className="text-indigo-500" /><span>Buyer protection</span></div>
              <div className="flex items-center gap-2 text-sm text-gray-600"><Clock size={16} className="text-indigo-500" /><span>Updated every 15min</span></div>
              <div className="flex items-center gap-2 text-sm text-gray-600"><Check size={16} className="text-green-500" /><span>Verified sellers</span></div>
            </div>
          </div>
        </div>
            ...
            {([
              ...
              <button
                ...
                className={`flex-1 min-w-[120px] py-4 text-sm font-medium transition-colors whitespace-nowrap px-4 ${
                  activeTab === tab.key ? 'text-indigo-600 border-b-2 border-indigo-600 bg-indigo-50/50' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="p-6">
            ...
            {activeTab === 'prices' && (
              <div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {sortedListings.map((listing, index) => (
                        <tr key={index} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${index === 0 ? 'bg-green-50/30' : ''}`}>
                          ...
                          <td className="text-right py-4 px-4">
                            ...
                            {listing.discount > 0 && <span className="ml-1 text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full">-{listing.discount}%</span>}
                          </td>
                          ...
                          <td className="text-right py-4 px-4">
                            <span className={`font-bold text-lg ${index === 0 ? 'text-green-600' : 'text-gray-900'}`}>{formatPrice(listing.totalCost, listing.currency)}</span>
                          </td>
                          ...
                          <td className="text-center py-4 px-4">
                            <span className={`inline-flex items-center text-xs font-medium px-2 py-1 rounded-full ${
                              ...
                              listing.stockLevel === 'low-stock' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
                            }`}>
                              {listing.stockLevel === 'in-stock' ? '✓ In Stock' : listing.stockLevel === 'low-stock' ? '⚡ Low' : '✗ Out'}
                            </span>
                          </td>
                          <td className="text-center py-4 px-4">
                          ...
                          <td className="text-center py-4 px-4">
                            <button
                              ...
                              disabled={redirecting === listing.store.id}
                              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-70 ${
                                index === 0 ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white hover:opacity-90 shadow-sm' : 'bg-indigo-600 text-white hover:bg-indigo-700'
                              }`}
                            >
                              {redirecting === listing.store.id ? (
                ...
                <div className="lg:hidden space-y-4">
                  {sortedListings.map((listing, index) => (
                    <div key={index} className={`relative border rounded-2xl p-5 transition-all ${index === 0 ? 'border-green-300 bg-green-50/30' : 'border-gray-100 bg-white'}`}>
                      {index === 0 && (
                        <div className="absolute -top-3 left-4 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-full">BEST TOTAL PRICE</div>
                      )}
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className={`text-xs font-medium px-2 py-1 rounded-full ${listing.stockLevel === 'in-stock' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => handleAffiliateClick(listing)}
                          disabled={redirecting === listing.store.id}
                          className={`px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-70 ${index === 0 ? 'bg-green-500 text-white' : 'bg-indigo-600 text-white'}`}
                        >
                          {redirecting === listing.store.id ? 'Loading...' : 'Buy Now'}
                        </button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-6 p-4 bg-amber-50 rounded-xl border border-amber-100">
                  <p className="text-xs text-amber-800">
                    ...
                    Prices shown include shipping. We always rank by best value regardless of commission rates.
                  </p>
                </div>
              </div>
            )}
              ...
              <div>
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                  ...
                  <div className="flex gap-2">
                    {([30, 90, 180] as const).map(range => (
                      <button key={range} onClick={() => setHistoryRange(range)}
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${historyRange === range ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                        {range}d
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                    <ResponsiveContainer width="100%" height={350}>
                      <LineChart data={chartData}>
                        ...
                        <Tooltip contentStyle={{ borderRadius: '12px', border: '1px solid #e5e7eb' }} formatter={(value: number) => [`₦${value.toLocaleString()}`, '']} />
                        <Legend />
                        {[...new Set(product.priceHistory.map(p => p.storeId))].map(storeId => {
                          const store = stores.find(s => s.id === storeId);
                          return <Line key={storeId} type="monotone" dataKey={storeId} name={store?.name || storeId} stroke={storeColors[storeId] || '#6366f1'} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />;
                        })}
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                  {(() => {
                    ...
                    const allPrices = product.priceHistory.filter(e => new Date(e.date) >= cutoff).map(e => e.currency === '$' ? e.price * 1500 : e.price);
                    ...
                    return (
                      <>
                        <div className="bg-white rounded-xl p-4 border border-gray-100"><p className="text-xs text-gray-500 mb-1">Current Best</p><p className="text-lg font-bold text-green-600">₦{current.toLocaleString()}</p></div>
                        <div className="bg-white rounded-xl p-4 border border-gray-100"><p className="text-xs text-gray-500 mb-1">Lowest Ever</p><p className="text-lg font-bold text-gray-900">₦{min.toLocaleString()}</p></div>
                        <div className="bg-white rounded-xl p-4 border border-gray-100"><p className="text-xs text-gray-500 mb-1">Highest</p><p className="text-lg font-bold text-gray-900">₦{max.toLocaleString()}</p></div>
                        <div className="bg-white rounded-xl p-4 border border-gray-100"><p className="text-xs text-gray-500 mb-1">Average</p><p className="text-lg font-bold text-gray-900">₦{Math.round(avg).toLocaleString()}</p></div>
                      </>
                    );
                  })()}
                </div>
            ...
            {activeTab === 'specs' && (
              <div>
                ...
                {specs.length > 6 && (
                  <button onClick={() => setShowAllSpecs(!showAllSpecs)} className="mt-4 flex items-center gap-1 text-sm text-indigo-600 font-medium hover:underline">
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-0.5">
                      {[...Array(5)].map((_, i) => <Star key={i} size={16} className={i < Math.round(avgRating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'} />)}
                    </div>
                    <span className="font-semibold">{avgRating.toFixed(1)}</span>
                  </div>
                </div>
                  ...
                  {product.ratings.map((review, i) => (
                    <div key={i} className="border border-gray-100 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 font-medium text-sm">{review.user.charAt(0)}</div>
                          <div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-0.5">
                            {[...Array(5)].map((_, j) => <Star key={j} size={12} className={j < review.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'} />)}
                          </div>
                          {review.verified && <span className="text-xs text-green-600 font-medium flex items-center gap-0.5"><Check size={10} /> Verified</span>}
                        </div>
                      </div>
                      <p className="text-sm text-gray-600">{review.comment}</p>
                      <div className="mt-2 flex items-center gap-3 text-xs text-gray-400">
                        <span>{review.helpful} found helpful</span>
                        <button onClick={() => addToast('Thanks for your feedback!', 'success')} className="text-indigo-600 hover:underline">Helpful</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {products.filter(p => p.id !== product.id && p.category === product.category).slice(0, 4).map(p => (
              <Link key={p.id} to={`/product/${p.id}`} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-all group">
                ...
                <div className="p-3">
                  ...
                  <p className="text-sm font-bold text-indigo-600 mt-1">{formatPrice(Math.min(...p.listings.map(l => l.totalCost)), p.listings[0].currency)}</p>
                </div>
              </Link>
            ))}
          </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={`e.g., ${Math.round(bestListing.totalCost * 0.9).toLocaleString()}`}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                  autoFocus
                />
              </div>
              <div>
                ...
                <div className="flex gap-3">
                  {['email', 'sms', 'push'].map(channel => (
                    <button
                      key={channel}
                      onClick={() => setAlertChannels(prev => prev.includes(channel) ? prev.filter(c => c !== channel) : [...prev, channel])}
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                        alertChannels.includes(channel) ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                    >
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowAlertModal(false)} className="flex-1 px-4 py-3 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Cancel</button>
                <button onClick={handleSetAlert} className="flex-1 px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl text-sm font-medium hover:opacity-90 transition-opacity">Set Alert</button>
              </div>
            </div>
          </div>
        </div>
